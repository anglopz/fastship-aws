...
resource "aws_lb_listener" "backend" {
  ...
  default_action {
    ...
    # Forward to target group if no certificate (fallback)
...
resource "aws_lb_listener" "backend_https" {
  ...
  port              = "443"
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS13-1-2-Res-PQ-2025-09"  # Use compatible policy that matches existing listeners
